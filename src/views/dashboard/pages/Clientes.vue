                      ...
                      <v-col cols="12" md="6">
                        ...
                        >
                          <template v-slot:activator="{ on, attrs }">
                            <v-text-field
                              v-model="tareas.fecha_nacimiento"
...
export default {
  ...
  methods: {
    mostrarClientes: async function () {
      try {
    ...
    async actualizarCliente() {
      ...
      var email = expR.test(this.tareas.correo);

      if (email) {
        if (datos) {
          await fetch("http://localhost:3000/cliente/actualizar", {
            method: "post",
        ...
        this.m_observacion = "Correo invalido";
        this.close();
        this.snackbar = true;
      }
    },
    ...
    deleteItem(item) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          //this.actualizarCliente(this.tareas);
          await fetch(
            "http://localhost:3000/cliente/actualizar?_id=" + item._id,
            {
              method: "delete",
              //body: JSON.stringify(this.tareas),
              headers: {
